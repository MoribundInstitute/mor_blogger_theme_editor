...
pub fn render_css_sockets(mut xml: String, config: &ThemeConfig) -> String {
    ...
    let body_stack = crate::config::fonts::resolve_font_stack_with_fallback(&config.typography.body_font_stack, false);
    let heading_stack = if config.typography.heading_font_stack.trim().is_empty() {
        body_stack.clone()
    } else {
        crate::config::fonts::resolve_font_stack_with_fallback(&config.typography.heading_font_stack, false)
    };
    let mono_stack = crate::config::fonts::resolve_font_stack_with_fallback(&config.typography.mono_font_stack, true);
    ...
    xml = xml.replace("{{LIGHT_BG_PANEL}}", &escape_attr(&light_palette.colors.bg_panel.to_css()));
    xml = xml.replace("{{LIGHT_BG_ELEVATED}}", &escape_attr(&light_palette.colors.bg_elevated.to_css()));
    xml = xml.replace("{{LIGHT_FG_BASE}}", &escape_attr(&light_palette.colors.fg_base));
    xml = xml.replace("{{LIGHT_FG_MUTED}}", &escape_attr(&light_palette.colors.fg_muted));
    xml = xml.replace("{{LIGHT_ACCENT}}", &escape_attr(&light_palette.colors.accent));
    xml = xml.replace("{{LIGHT_BORDER}}", &escape_attr(&light_palette.colors.border));
    xml = xml.replace("{{LIGHT_PANEL_BORDER_WIDTH}}", &escape_attr(&light_palette.colors.panel_border_width));
    xml = xml.replace("{{LIGHT_GLOW_SPREAD}}", &escape_attr(&light_palette.colors.glow_spread));
    xml = xml.replace("{{LIGHT_HOVER_SCALE}}", &escape_attr(&light_palette.colors.hover_scale));
    xml = xml.replace("{{LIGHT_PANEL_BORDER_IMAGE_URL}}", &escape_attr(&light_palette.colors.panel_border_image_url));
    xml = xml.replace("{{LIGHT_PANEL_BORDER_IMAGE_SLICE}}", &escape_attr(light_panel_border_image_slice));
    xml = xml.replace("{{LIGHT_PANEL_BORDER_IMAGE_REPEAT}}", &escape_attr(light_panel_border_image_repeat));
    ...
    xml = xml.replace("{{DARK_BG_ELEVATED}}", &escape_attr(&dark_palette.colors.bg_elevated.to_css()));
    xml = xml.replace("{{DARK_FG_BASE}}", &escape_attr(&dark_palette.colors.fg_base));
    xml = xml.replace("{{DARK_FG_MUTED}}", &escape_attr(&dark_palette.colors.fg_muted));
    ...
    xml = xml.replace("{{DARK_PANEL_BORDER_WIDTH}}", &escape_attr(&dark_palette.colors.panel_border_width));
    xml = xml.replace("{{DARK_GLOW_SPREAD}}", &escape_attr(&dark_palette.colors.glow_spread));
    xml = xml.replace("{{DARK_HOVER_SCALE}}", &escape_attr(&dark_palette.colors.hover_scale));
    xml = xml.replace("{{DARK_PANEL_BORDER_IMAGE_URL}}", &escape_attr(&dark_palette.colors.panel_border_image_url));
    xml = xml.replace("{{DARK_PANEL_BORDER_IMAGE_SLICE}}", &escape_attr(dark_panel_border_image_slice));
    xml = xml.replace("{{DARK_PANEL_BORDER_IMAGE_REPEAT}}", &escape_attr(dark_panel_border_image_repeat));
    ...
    xml = xml.replace("{{SCALE_RATIO}}", &escape_attr(&config.typography.scale_ratio));
    xml = xml.replace("{{LINE_HEIGHT}}", &escape_attr(&config.typography.line_height));
    xml = xml.replace("{{HEADING_WEIGHT}}", &escape_attr(&config.typography.heading_weight));
    
    xml = xml.replace("{{BTN_RADIUS}}", &escape_attr(&config.buttons.radius));
    xml = xml.replace("{{BTN_BORDER_WIDTH}}", &escape_attr(&config.buttons.border_width));
    xml = xml.replace("{{BTN_TEXT_TRANSFORM}}", &escape_attr(&config.buttons.text_transform));

    xml = xml.replace("{{PANEL_BORDER_WIDTH}}", &escape_attr(panel_border_width));
    xml = xml.replace("{{CONTAINER_BORDER_WIDTH}}", &escape_attr(panel_border_width));
    xml = xml.replace("{{FRAME_BORDER_WIDTH}}", &escape_attr(panel_border_width));

    xml = xml.replace("{{PANEL_BORDER_IMAGE_URL}}", &escape_attr(&config.colors.panel_border_image_url));
    xml = xml.replace("{{PANEL_BORDER_IMAGE_SLICE}}", &escape_attr(panel_border_image_slice));
    xml = xml.replace("{{PANEL_BORDER_IMAGE_REPEAT}}", &escape_attr(panel_border_image_repeat));
    ...
    let soft_glow = format!("0 0 calc({} / 2) {}", escape_attr(glow_spread), escape_attr(&color_accent_soft));
    let strong_glow = format!("0 0 {} {}", escape_attr(glow_spread), escape_attr(&config.colors.accent));
    ...
    xml = xml.replace("{{BACKGROUND_TILE_URL}}", &escape_attr(&background_tile_url));

    xml = xml.replace("{{ICON_SIDEBAR_LEFT}}",  &icon_or_default(&config.icons.sidebar_left,  DEFAULT_ICON_SIDEBAR_LEFT));
    xml = xml.replace("{{ICON_SIDEBAR_RIGHT}}", &icon_or_default(&config.icons.sidebar_right, DEFAULT_ICON_SIDEBAR_RIGHT));
    xml = xml.replace("{{ICON_PANEL_CLOSE}}",   &icon_or_default(&config.icons.panel_close,   DEFAULT_ICON_PANEL_CLOSE));
    xml = xml.replace("{{ICON_SEARCH}}",        &icon_or_default(&config.icons.search,        DEFAULT_ICON_SEARCH));
    xml = xml.replace("{{ICON_MENU}}",          &icon_or_default(&config.icons.menu,          DEFAULT_ICON_MENU));

    // Remaining structural sizing
    xml = xml.replace("{{HEADER_LOGO_SIZE}}", "72px");
    xml = xml.replace("{{HEADER_LOGO_RADIUS}}", &escape_attr(&config.buttons.radius));
    ...
    xml = xml.replace("{{HEADER_LOGO_RADIUS_MOBILE}}", &escape_attr(&config.buttons.radius));
    ...
    xml = xml.replace("{{HEADER_LOGO_RADIUS_SMALL}}", &escape_attr(&config.buttons.radius));
