    ...
    rsx! {
        section {
            ...
            style: "position: fixed; top: 88px; left: 50%; transform: translateX(-50%); \
                    z-index: 3200; width: min(720px, calc(100vw - 48px)); height: min(600px, calc(100vh - 120px)); \
            ...
            header {
                style: "flex: 0 0 auto; display: flex; align-items: center; justify-content: space-between; \
                        padding: 14px 18px; background: #0b0a09; border-bottom: 1px solid rgba(236, 231, 218, 0.18);",
                div {
                    p { style: "margin: 0 0 2px; font-family: monospace; font-size: 0.62rem; letter-spacing: 0.22em; color: #8c8678;", "Moribund · Module Registry" }
                    h2 { style: "margin: 0; font-size: 1.12rem; font-weight: 600; color: #f4f1ea;", "Plugin Manager" }
                }
                button {
