    ...
    rsx! {
        div { class: "editor-panel-content", style: "display: flex; flex-direction: column; gap: 16px;",
            ...
            div { style: "display: flex; flex-direction: column; gap: 12px;",
                CompactSelect { label: "Header Variant", val: pack.header_variant.clone(), options: vec![("mor", "Mor (Default)"), ("gtk_headerbar", "GTK4 Headerbar")], on_change: { let c = current_config.clone(); let f = on_apply_theme.clone(); move |v| { let mut nc = c.clone(); nc.template_pack.header_variant = v; f.call(nc); } } }
                CompactSelect { label: "Main Canvas", val: pack.main_variant.clone(), options: vec![("sidebars", "Three Column (Sidebars)"), ("single_column", "Single Column")], on_change: { let c = current_config.clone(); let f = on_apply_theme.clone(); move |v| { let mut nc = c.clone(); nc.template_pack.main_variant = v; f.call(nc); } } }
